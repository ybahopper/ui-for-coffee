...
function lib.new(config)
    ...
    function window:addTab(tabConfig)
        ...
        function tab:addCard(cardConfig)
            ...
            function cardObj:addToggle(toggleConfig)
                ...
                toggle.MouseEnter:Connect(function()
                    TweenService:Create(toggle, TWEEN_INFO, { BackgroundTransparency = math.max(toggle.BackgroundTransparency - 0.08, 0) }):Play()
                end)
                toggle.MouseLeave:Connect(function()
                    local t = state and TOGGLE_ON.bgTransparency or TOGGLE_OFF.bgTransparency
                    TweenService:Create(toggle, TWEEN_INFO, { BackgroundTransparency = t }):Play()
                end)
                row.Parent = card
                updateSeparators()
        ...
        btn.MouseEnter:Connect(function()
            TweenService:Create(btn, TWEEN_INFO, { BackgroundTransparency = math.max(btn.BackgroundTransparency - 0.08, 0) }):Play()
        end)
        btn.MouseLeave:Connect(function()
            local t = (tab == activeTab) and 0.72 or 1
            TweenService:Create(btn, TWEEN_INFO, { BackgroundTransparency = t }):Play()
        end)
        btn.MouseButton1Click:Connect(function()
            setActiveTab(tab)
        end)
    ...
    function window:notify(notifConfig)
        ...
        -- wrap notif in a clip container that expands from 0
        local notifH = notif.Size.Y.Offset
        local wrapper = Instance.new("Frame")
        wrapper.Name = "notif_wrapper"
        wrapper.BackgroundTransparency = 1
        wrapper.BorderSizePixel = 0
        wrapper.ClipsDescendants = true
        wrapper.Size = UDim2.new(1, 0, 0, 0)
        wrapper.Parent = notifications_container
        ...
        -- expand wrapper + fade in
        local notifInTi = TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
        TweenService:Create(wrapper, notifInTi, { Size = UDim2.new(1, 0, 0, notifH) }):Play()
        TweenService:Create(notif, notifInTi, { BackgroundTransparency = notifSnap.bg }):Play()
        for desc, snap in pairs(notifChildSnap) do
        ...
        task.delay(duration, function()
            ...
            updateNotifBadge()
            -- collapse wrapper + fade out
            local notifOutTi = TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
            TweenService:Create(wrapper, notifOutTi, { Size = UDim2.new(1, 0, 0, 0) }):Play()
            TweenService:Create(notif, notifOutTi, { BackgroundTransparency = 1 }):Play()
            for desc, snap in pairs(notifChildSnap) do
    ...
    do
        ...
        if search_closed_btn and search_open_frame then
            ...
            local function closeSearch()
                if not search_open_frame.Visible then return end
            ...
            if searchClose then searchClose.MouseButton1Click:Connect(closeSearch) end
